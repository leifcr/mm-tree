...
class TestMongomapperActsAsTree < Test::Unit::TestCase
  context "Tree" do
    ...
    context "node" do
      ...
      #   end
        ...
        should "move children on save and don't touch timestamps for children" do
          ...
          puts "#{@node_2_4_1.name} #{@node_2_4_1.updated_at.to_f} - #{before_updated_at_2_4_1.to_f}"
          puts "#{@node_2_4_1_1.name} #{@node_2_4_1_1.updated_at.to_f} - #{before_updated_at_2_4_1_1.to_f}"

          assert @node_2_4_1.created_at == before_created_at_2_4_1, "Created timestamp should not be altered for #{@node_2_4_1.name} Got: #{@node_2_4_1.created_at} Expected: #{before_created_at_2_4_1}"
          assert @node_2_4_1.updated_at == before_updated_at_2_4_1, "Updated timestamp should not be altered for #{@node_2_4_1.name} Got: #{@node_2_4_1.updated_at} Expected: #{before_updated_at_2_4_1}"
          assert @node_2_4_1_1.created_at == before_created_at_2_4_1_1, "Created timestamp should not be altered for #{@node_2_4_1_1.name} Got: #{@node_2_4_1_1.created_at} Expected: #{before_created_at_2_4_1_1}"
          assert @node_2_4_1_1.updated_at == before_updated_at_2_4_1_1, "Updated timestamp should not be altered for #{@node_2_4_1_1.name} Got: #{@node_2_4_1_1.updated_at} Expected: #{before_updated_at_2_4_1_1}"
        end
    ...
    #     assert @node_1.tree_keys        == Hash[:nv => 1, :dv => 1, :snv => 2, :sdv => 1],       "Keys for #{@node_1.name} is wrong. got #{@node_1.tree_keys}, expected: #{Hash[:nv => 1, :dv => 1, :snv => 2, :sdv => 1]}"
    #     assert @node_2.tree_keys        == Hash[:nv => 2, :dv => 1, :snv => 3, :sdv => 1],       "Keys for #{@node_2.name} is wrong. got #{@node_2.tree_keys}, expected: #{Hash[:nv => 2, :dv => 1, :snv => 3, :sdv => 1]}"
    #     assert @node_2_1.tree_keys      == Hash[:nv => 5, :dv => 2, :snv => 8, :sdv => 3],       "Keys for #{@node_2_1.name} is wrong. got #{@node_2_1.tree_keys}, expected: #{Hash[:nv => 5, :dv => 2, :snv => 8, :sdv => 3]}"
    #     assert @node_1_3.tree_keys      == Hash[:nv => 7, :dv => 4, :snv => 9, :sdv => 5],       "Keys for #{@node_1_3.name} is wrong. got #{@node_1_3.tree_keys}, expected: #{Hash[:nv => 7, :dv => 4, :snv => 9, :sdv => 5]}"
    #     assert @node_2_4.tree_keys      == Hash[:nv => 14, :dv => 5, :snv => 17, :sdv => 6],     "Keys for #{@node_2_4.name} is wrong. got #{@node_2_4.tree_keys}, expected: #{Hash[:nv => 14, :dv => 5, :snv => 17, :sdv => 6]}"
    #     assert @node_2_4_1.tree_keys    == Hash[:nv => 31, :dv => 11, :snv => 48, :sdv => 17],   "Keys for #{@node_2_4_1.name} is wrong, got #{@node_2_4_1.tree_keys}, expected: #{Hash[:nv => 31, :dv => 11, :snv => 48, :sdv => 17]}"
    #     assert @node_2_4_3.tree_keys    == Hash[:nv => 65, :dv => 23, :snv => 82, :sdv => 29],   "Keys for #{@node_2_4_3.name} is wrong, got #{@node_2_4_3.tree_keys}, expected: #{Hash[:nv => 65, :dv => 23, :snv => 82, :sdv => 29]}"
    #     assert @node_2_4_1_1.tree_keys  == Hash[:nv => 79, :dv => 28, :snv => 127, :sdv => 45], "Keys for #{@node_2_4_1_1.name} is wrong, got #{@node_2_4_1_1.tree_keys}, expected: #{Hash[:nv => 79, :dv => 28, :snv => 127, :sdv => 45]}"
    #   end
